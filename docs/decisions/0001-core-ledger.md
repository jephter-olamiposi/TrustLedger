# Decision 0001: Core Ledger Architecture and Financial Invariants
...
- **Decision 0002** — account/transfer indexes are `BTreeMap` for deterministic iteration and replay.
- **Decision 0003** — journal payloads are versioned; replay fails clearly on corrupted or out-of-order journals.
- **Decision 0004** — availability reads return zero when overdrawn (view, not guard); `close_account` owns the account lifecycle with a journal event.
- **Decision 0005** — transfer-state gates stay runtime-checked and typed; typestate seeds deferred.
- **Decision 0006** — state-transition ordering is compute-then-write (state flip last); event timestamps are monotonic under `LedgerError::TimestampBehindPrior`.
