# Decision 0007: Write-Ahead Log — Framing, Durability, and Recovery
...
Decision 0003 already fixed the ledger's *event* format (postcard schema version 1).
...
1. **compute** — `ledger.prepare_batch(&mut self, &[Transfer])` speculatively
  ...
  the speculation back — a prepare changes no state (Decision 0006's
   compute-write ordering, reused, not duplicated);
2. **durable** — the caller encodes the batch with `ledger_core::codec`
  (postcard, Decision 0003) and appends the opaque byte slice to the wal;
...
| version | 1 | `u8` = 1 (Decision 0003's schema version, incremented with it) |
...
under Decision 0003's versioning without touching the storage layer. A malformed
...
- The ledger stays pure and deterministic (Decision 0002, Decision 0003): replay from
...
- Decision 0003 (journal versioning), Decision 0006 (compute-then-write ordering).